...
function RatingDetails(props){
   ...
   const [ratings,setratings]=useState("props.ratingsValue");
  ...
  useEffect(()=>{
    ...
    setratings(props.ratingsValue);
    setImage(props.imageValue);
 
},[props.roomClassValue,props.roomTypeValue,props.NoOfRoomsValue,props.costValue,props.ratingsValue,,props.imageValue]);
   const arr=[roomClass,roomType,NoOfRooms,cost,ratings,image];
    ...
    return(
        <div>
         ...
         <label htmlFor="cost" style={{color:"black"}}>Cost:</label>
          ...
          <label htmlFor="ratings" style={{color:"black"}}>Ratings:</label>
          <input id="ratings" type="text"  defaultValue={props.ratingsValue} onChange={(event) => setratings(event.target.value)}class="form-control my-3" placeholder="Ratings" />
